...
//if ($(this).find('hr').length === 0) $(this).append('<hr>' + $(this).data('date'))
	...
	function _GetSeparator(curDate, prevDate) {
		var nSeparatorSize = 0;
		var aSeparators = _GetSettingsParam('separators', 'list');
		for (var i in aSeparators) {

			if (curDate.getMonth() !== prevDate.getMonth() || curDate.getYear() !== prevDate.getYear()) {
				if (aSeparators[i].separateItemType === 'month') {
					nSeparatorSize = aSeparators[i].gapSize;
					continue;
				}

				if (curDate.getDate() !== prevDate.getDate()) {
					if (aSeparators[i].separateItemType === 'day') {
						nSeparatorSize = aSeparators[i].gapSize;
						continue;
					}	
				}
	...
	$.fn.akqaTimelineV = function(method) {
	    if ( methods[method] ) {
